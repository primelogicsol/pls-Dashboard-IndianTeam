...
import { fetchOutsourcedProjects, getProjectsForSelectFreelancer } from "@/lib/api/projects";
...
export default function MyProjectsPage() {
  ...
  const limit = 5; //The number of projects per page

  useEffect(() => {
    const getProjects = async () => {
      setLoading(true);
      try {
        const response = await getProjectsForSelectFreelancer(
          pagination.page,
          limit
        );
    ...
    getProjects();
  }, [pagination.page]); // Fetch projects when page or filters change
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects?.map((project) => (
            <Card key={project.id} className="flex flex-col">
              ...
              <CardFooter>
                <Link
                  href={`/dashboard/freelancer/my-projects/${project.projectSlug}`}
                  className="w-full"
                >
